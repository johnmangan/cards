...
namespace cards
{
  ...
  const std::string stmt_select = "SELECT Location FROM AssetLocator WHERE Name = ?;";
  const std::string stmt_select_lod = "SELECT Location FROM AssetLocator WHERE Name = ? AND LOD = ?;";
  const std::string stmt_select_max_lod = "SELECT MAX(LOD) FROM AssetLocator WHERE Name = ?;";
